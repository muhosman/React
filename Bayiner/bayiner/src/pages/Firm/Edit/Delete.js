...
function Delete() {
  const tagText = "font-SemiBold ml-2  xl:text-XL md:text-BASE text-SM ";
  const InputCheckBox = "w-6 h-6 ml-3 mb-2";
  const tagIcon = "xl:w-9 xl:h-9 w-8 h-8 text-fourth";
  const inputTag =
    "w-full h-11 input rounded-md shadow border-1 p-3 bg-slate-200 xl:text-XL md:text-BASE text-SM text-center";
  const button =
    "xl:text-XL md:text-BASE text-SM items-center gap-4 text-white flex bg-fourth rounded-md px-4 py-3 transition-all duration-300 active:scale-90";
          ...
          <div className=" flex gap-4 w-full">
            ...
            >
              ...
              <div className=" flex flex-col justify-center gap-2">
                <div className="flex gap-2 items-center">
                  <RiLockPasswordLine className={`${tagIcon}`} />
                  <p className={`${tagText}`}>Şifre Onay</p>
                </div>
                <input
                  required
                  type="password"
              ...
              <button className={`${button}`}>
                <p className=" ">Firmayı Sil</p>
                <MdOutlineDeleteOutline
                  className={`${buttonIcon} ${true ? "animate-bounce" : ""}`}
                />
              </button>
            </form>
          </div>
