...
const TownPage = function () {
  ...
  const handleModel = (e) => {
    e.preventDefault();
    setInputFieldName({ name: "İlçe İsmi" });
    ...
    return (
      <form onSubmit={handleModel} className=" flex flex-col gap-6 min-w-max  ">
        ...
        <div className="flex flex-col gap-6 min-w-max px-8 pb-4">
          <text className={`text-white pl-3 ${styles.textTitle}`}>
            İlçe İsmi:
          </text>
          <input
  ...
  const filteredProductType = (isSearch ? filteredData : Town)?.filter(
    (Town, index) => {
      if (
  ...
  const content = filteredProductType?.map((Town) => {
    return (
      <div className={`bg-white rounded-md shadow-lg`}>
        <div
          className={`text-center w-full p-4 rounded-t-xl bg-fourth text-white relative`}
        >
          <p className={`${styles.cardTitle}`}>İlçe Bilgisi</p>
        </div>

        <div className=" p-10">
          <div className=" flex flex-col gap-4 ">
            <div>
              <text className={` ${styles.textTitle}`}>İlçe İsmi: </text>
              <text className={`${styles.text}`}>{Town.name}</text>
            </div>
  ...
  return (
    ...
    -translate-x-1/2"
      ...
      <div className=" w-full flex flex-col gap-4">
        <div className="flex max-md:flex-col gap-3 md:items-center text-fourth w-fit">
          <div className=" flex gap-2">
            <div className={`${styles.PageHeader}`}>İlçe Listesi</div>
            <div
      ...
      ) : TownResult.error || Town?.length === 0 ? (
        <div className=" text-SemiBold">İlçe bulunamadı</div>
      ) : (
        <>
          <div className="flex flex-col items-center z-0 ">
            <PaginationBar
              elements={isSearch ? filteredData : Town}
      ...
      {designModel && (
        <>
          ...
          -translate-x-1/2  w-fit max-h-[41rem]
          `}
          >
            <div
