...
import DataSearchBar from "./ProductTypeSearchBar";
import styles from "../../../../CustomStyles";
import Confirm from "../../../../components/Confirm";

const ProductTypePage = function () {
  const { auth } = useAuth();
  ...
  const [Update, resultUpdate] = useUpdateProductTypeMutation();
  const [Delete, resultDelete] = useDeleteProductTypeMutation();
  const [Add, resultAdd] = useAddProductTypeMutation();
  ...
  const handleModal = (e) => {
    e.preventDefault();
    setInputFieldName({ name: "Ürün Tipi" });
  ...
  const filteredProductType = (isSearch ? filteredData : ProductType)?.filter(
    (ProductType, index) => {
      if (
  ...
  const content = filteredProductType?.map((ProductType) => {
    return (
      <div className={`bg-white rounded-md shadow-lg`}>
        <div
          className={`text-center w-full p-4 rounded-t-xl bg-fourth text-white relative`}
        >
          <p className={`${styles.cardTitle}`}>Ürün Tipi</p>
        </div>
        <div className=" p-10">
          <div className=" flex flex-col gap-4 ">
            <div>
              <text className={` ${styles.textTitle}`}>Ürün Tipi: </text>
              <text className={`${styles.text}`}>{ProductType.name}</text>
            </div>
          </div>
  ...
  return (
    ...
    -translate-x-1/2"
      ...
      <div className=" w-full flex flex-col gap-4">
        <div className="flex max-md:flex-col gap-3 md:items-center text-fourth w-fit">
          <div className=" flex gap-2">
            <div className={`${styles.PageHeader}`}>Ürün Listesi</div>
            <div
        ...
        >
          <DataSearchBar Data={ProductType} handleSearch={handleSearch} />
        </div>
      </div>

      {ProductTypeResult.isFetching ? (
      ...
      ) : ProductTypeResult.error || ProductTypeResult.data?.results === 0 ? (
        <div className=" text-SemiBold">Ürün bulunamadı</div>
      ) : (
        <>
          <div className="flex flex-col items-center">
            <PaginationBar
              elements={isSearch ? filteredData : ProductType}
